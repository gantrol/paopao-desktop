...
contextBridge.exposeInMainWorld('paopao', {
  ...
  conversations: {
    ...
    clear: (conversationId) => ipcRenderer.invoke('conversation:clear', conversationId),
    updateMeta: (payload) => ipcRenderer.invoke('conversation:update-meta', payload),
  },
  messages: {
  ...
  ai: {
    ...
    triggerConversationBots: (payload) => ipcRenderer.invoke('ai:trigger-conversation-bots', payload),
    triggerMachineRun: (payload) => ipcRenderer.invoke('ai:trigger-machine-run', payload),
    cancelMachineRun: (payload) => ipcRenderer.invoke('ai:cancel-machine-run', payload),
    onConversationBotStream: (listener) => {
      const handler = (_event, payload) => listener(payload);
      ipcRenderer.on('ai:conversation-bot-stream', handler);
      return () => ipcRenderer.removeListener('ai:conversation-bot-stream', handler);
    },
  ...
  settings: {
    ...
    saveAiProvider: (payload) => ipcRenderer.invoke('settings:save-ai-provider', payload),
    listBots: (payload) => ipcRenderer.invoke('settings:list-bots', payload),
    listBotConversations: (botId) => ipcRenderer.invoke('settings:list-bot-conversations', botId),
    saveBot: (payload) => ipcRenderer.invoke('settings:save-bot', payload),
    getUserProfile: () => ipcRenderer.invoke('settings:get-user-profile'),
    saveUserProfile: (payload) => ipcRenderer.invoke('settings:save-user-profile', payload),
  },
  bots: {
    ensureDirectConversation: (botId) => ipcRenderer.invoke('bots:ensure-direct-conversation', botId),
    saveBinding: (payload) => ipcRenderer.invoke('conversation:save-bot-binding', payload),
  },
  system: {
