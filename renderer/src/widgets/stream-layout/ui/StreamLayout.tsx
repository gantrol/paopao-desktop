...
type StreamListRow = { type: 'channel'; channel: ChatChannel } | { type: 'folded-entry' };
  ...
  return (
    <>
      <div className="pane-edge-wrapper">
                    ...
                    if (row.type === 'folded-entry') {
                      return (
                        <button
                          ...
                          title={listViewMode === 'folded' ? '返回流动列表' : '打开泡泡折叠空间'}
                          aria-label={listViewMode === 'folded' ? '返回流动列表' : '打开泡泡折叠空间'}
                        >
                          <span className="stream-list-collapsed-avatar-shell stream-list-collapsed-avatar-shell--ghost">
                            <span className="stream-list-collapsed-avatar-copy">叠</span>
                          </span>
                          {foldedChannelCount > 0 ? <span className="stream-list-collapsed-badge">{foldedChannelCount}</span> : null}
                        </button>
                      );
                    }
                    ...
                    return (
                      ...
                      >
                        <span className="stream-list-collapsed-avatar-shell">
                          {renderChannelAvatar(channel, 'h-10 w-10 rounded-[18px]', 'text-[20px] font-semibold text-white')}
                        </span>
                        {channel.isPinned ? (
                          <span className="stream-list-collapsed-corner-badge" title="置顶" aria-hidden="true">
                            <PinnedStatusGlyph size={12} />
                          </span>
                        ) : channel.isStalled ? (
                          <span className="stream-list-collapsed-corner-badge" title="停滞" aria-hidden="true">
            ...
            <>
              <div className="nav-bar">
                <div
                  ...
                  title="新建会话"
                >
                  <svg viewBox="0 0 24 24" width="22" height="22" fill="var(--text-primary)"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" /></svg>
                </div>
                ...
                {rows.length === 0 ? (
                  ...
                  if (row.type === 'folded-entry') {
                    return (
                      ...
                      >
                        <div>
                          <div className="text-sm font-semibold text-[var(--text-primary)]">泡泡折叠空间</div>
                          <div className="mt-1 text-xs text-[var(--text-secondary)]">{listViewMode === 'folded' ? '点此返回流动列表' : '这里收纳你主动折叠的泡泡流'}</div>
                        </div>
                        <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-[var(--accent)]">{listViewMode === 'folded' ? '返回' : foldedChannelCount}</span>
                      </button>
                    );
                  }

                  const channel = row.channel;
                  const lastChannelMessage = channel.messages[channel.messages.length - 1] || null;
                  return (
                    <div
                      key={channel.id}
                      ...
                      onDoubleClick={() => onOpenChannelSettings(channel.id)}
                    >
                      {renderChannelAvatar(channel, 'chat-list-avatar h-12 w-12 rounded-2xl', 'text-[17px] font-semibold text-white')}
                      <div className="chat-list-info">
                        <div className="flex items-center gap-2">
                          <div className="chat-list-title">{channel.title}</div>
                          {channel.isPinned ? (
                            <span className="status-chip status-chip--sm status-chip--accent chat-list-status-badge" title="置顶" aria-label="置顶">
                              <PinnedStatusGlyph size={12.5} />
                            </span>
                          ) : null}
                          {channel.isFolded ? <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-[var(--accent)]">折叠</span> : null}
                          {channel.isStalled ? (
                            <span className="status-chip status-chip--sm chat-list-status-badge" title="停滞" aria-label="停滞">
                              <StalledStatusGlyph size={12.5} />
                            </span>
                          ) : null}
                        </div>
                        <div className={`chat-list-desc ${isMessageStreaming(lastChannelMessage) ? 'is-streaming' : ''}`}>{channel.lastMsg || '点击进入这个泡泡流'}</div>
                      </div>
                      <div className="chat-list-time">{formatConversationListTime(getChannelRankTime(channel)) || channel.lastTime}</div>
                    </div>
                  );
                })}
              </div>
        ...
        <ResizeHandle className="pane-resize-handle pane-resize-handle--list desktop-only" onDrag={onResize} ariaLabel="调整左侧会话列表宽度" limit={limit} />
      </div>
    </>
  );
}
...
const StreamMessageArea = memo(function StreamMessageArea({
  ...
  const messageEntries = useMemo(() => sortedMessages.map((msg, index) => (
    <ConversationFeedEntry
      key={msg.id}
      rowId={`msg-${msg.id}`}
      message={msg}
      timestampLabel={getConversationDividerLabel(msg.time, sortedMessages[index - 1]?.time || null)}
    ...
    >
      ...
      {navStackDepth > 0 && mobileView === 'chat' ? (
        <div className="nav-back-float show" onClick={onNavBack}>
          <svg viewBox="0 0 24 24"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z" /></svg><span>返回</span>
        </div>
      ) : null}

      <input type="file" ref={streamPhotoInputRef} accept="image/*" multiple style={{ display: 'none' }} onChange={onStreamPhotoInputChange} />
      <input type="file" ref={streamFileInputRef} multiple style={{ display: 'none' }} onChange={onStreamFileInputChange} />
    </div>
  );
});
