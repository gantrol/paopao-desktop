...
import { createEmptyDraftState, classifyMessageFile, type DraftState, type QuoteState } from '@/features/send-message/model/draft';
import {
...
import { buildMessagePreviewText, type BubbleBlock, type MessageData } from '@/entities/message';
...
import { DEFAULT_STREAM_AVATAR_PRESET, USER_AVATAR } from '@/shared/config/avatar';
...
import { buildBubbleLink, resolveBubbleLinkBlocksForSubmit } from '@/shared/lib/bubble-link';
import {
  buildQuote,
  clearChatMessages,
  createChat,
...
type SortingSourceLocator = (payload: SortingSourceLocatorPayload) => void | Promise<void>;

function countRepliesByMessage(messages: MessageData[]) {
  const counts: Record<string, number> = {};
  messages.forEach((message) => {
    if (!message.replyToMessageId) return;
    counts[message.replyToMessageId] = (counts[message.replyToMessageId] || 0) + 1;
  });
  return counts;
}
...
export default function AppRouter() {
  ...
  const [mobileView, setMobileView] = useState<'list' | 'chat' | 'thread' | 'sorting' | 'factory'>('list');
  ...
  const [userProfile, setUserProfile] = useState<UserProfileRecord>({ avatarUrl: USER_AVATAR });
  const [pendingFiles, setPendingFiles] = useState<File[]>([]);
  const [showFileConfirm, setShowFileConfirm] = useState(false);
  const [pendingLink, setPendingLink] = useState<string | null>(null);
  const [showLinkConfirm, setShowLinkConfirm] = useState(false);
  const [contextMenu, setContextMenu] = useState<ContextMenuState>({ show: false, x: 0, y: 0, msgId: null, blockId: undefined, subItemIndex: undefined, content: null, media: null, mediaType: null });
  ...
  const [editingThreadMessageId, setEditingThreadMessageId] = useState<string | null>(null);
  const [forwardingMessageId, setForwardingMessageId] = useState<string | null>(null);
  const [streamSettingsOpen, setStreamSettingsOpen] = useState(false);
  const [streamSettingsChatId, setStreamSettingsChatId] = useState<string | null>(null);
  ...
  const [pendingJumpTarget, setPendingJumpTarget] = useState<PendingJumpTarget | null>(null);
  const [contextMenuSubmenu, setContextMenuSubmenu] = useState<'machines' | null>(null);


  const listPane = useBoundedPaneSize({ initial: 304, min: LIST_PANE_MIN, max: LIST_PANE_MAX });
  const threadPane = useBoundedPaneSize({ initial: 348, min: THREAD_PANE_MIN, max: THREAD_PANE_MAX });
  ...
  const previousMessageContextRef = useRef<{ conversationId: string | null; count: number }>({ conversationId: null, count: 0 });
  ...
  const currentChannel = channels.find((item) => item.id === selectedChatId) || channels[0];
  const streamSettingsChannel = channels.find((item) => item.id === streamSettingsChatId) || null;
  const manualMachineBots = useMemo(
    () => factoryBots.filter((bot) => bot.enabled && (bot.runtimeType === 'llm' || bot.runtimeType === 'external-codex')),
    [factoryBots],
  );
  const messages = currentChannel?.messages || [];
  const threadConversation = channels.find((item) => item.id === threadConversationId) || null;
  ...
  const threadSortedMessages = useMemo(
    () => [...threadMessages].sort((left, right) => (left.time || 0) - (right.time || 0)),
    [threadMessages],
  );
  ...
  const openThreadPane = useCallback((context: ThreadContext, options?: { preserveStack?: boolean }) => {
  ...
  const closeThreadPane = useCallback(() => {
    const nextMobileView = activeChat === 'factory'
      ? 'factory'
  ...
  const openCurrentChatThreadPane = useCallback((messageId: string, blockId?: string) => {
  ...
  const openSortingThreadPane = useCallback((payload: SortingSourceLocatorPayload) => {
    openThreadPane({
      origin: 'sorting',
  ...
  const registerSortingSourceLocator = useCallback((locator: SortingSourceLocator | null) => {
  ...
  const openAttachmentWithDefaultApp = useCallback(async (target: string) => {
    if (!bridge) return;
    try {
  ...
  const applyConversation = useCallback((record: ChatRecord, options?: { select?: boolean }) => {
  ...
  const maybeTriggerAutoMachines = useCallback(async (
    conversation: ChatRecord,
    messageId?: string | null,
  ) => {
    const resolvedMessageId = messageId || conversation.messages[conversation.messages.length - 1]?.id || '';
    if (!resolvedMessageId) return;
    const sourceMessage = conversation.messages.find((message) => message.id === resolvedMessageId) || null;
    if (!sourceMessage || sourceMessage.role !== 'me' || sourceMessage.replyToMessageId) {
      return;
    }
    ...
    if (replyResult.status === 'fulfilled' && replyResult.value?.conversation) {
  ...
  const persistConversationMeta = useCallback(async (payload: {
    conversationId: string;
    title?: string;
    avatarPreset?: string;
    avatarUrl?: string;
    ...
    const nextChannel = applyConversation(updated, { select: options?.select });
  ...
  const buildCurrentConversationUiState = useCallback((overrides?: {
    ...
    return {
      ...
      thread: {
        ...
        isCollapsed: Boolean(isCurrentConversationThreadOpen && isThreadCollapsed),
      },
    };
  }, [
    currentChannel,
  ...
  const persistCurrentConversationUiState = useCallback((options?: {
    immediate?: boolean;
    messageScrollTop?: number;
  }) => {
    if (!currentChannel) return;
    ...
    const currentPersistedState = getConversationUiState(currentChannel.metadata);
    if (JSON.stringify(currentPersistedState) === JSON.stringify(nextUiState)) {
      return;
    }
    ...
    const loadChannels = async () => {
      try {
        ...
        setSelectedChatId((prev) => (
          prev
          || loaded.find((channel) => (channel.lifecycleStatus || 'flowing') === 'flowing' && !channel.isFolded)?.id
          || loaded[0]?.id
          || ''
        ));
      } catch (error) {
  ...
  useLayoutEffect(() => {
    ...
    if (currentConversationUiState.thread.open && currentConversationUiState.thread.messageId) {
      setThreadContext({
        origin: 'chat',
        conversationId: currentChannel.id,
    ...
    setIsThreadCollapsed(Boolean(currentConversationUiState.thread.open && currentConversationUiState.thread.isCollapsed));
    setNavStack([]);
    setThreadNavStack([]);
  }, [activeChat, currentChannel?.id]);

  useEffect(() => {
    if (!currentChannel || activeChat !== 'assistant' || mobileView === 'sorting') return;
    const conversationChanged = previousMessageContextRef.current.conversationId !== currentChannel.id;
    const nextMessageCount = messages.length;

    window.setTimeout(() => {
      if (!msgAreaRef.current || !currentChannel) return;
      if (restoreConversationRef.current === currentChannel.id || conversationChanged) {
        const targetScrollTop = currentConversationUiState.messageScrollTop;
        msgAreaRef.current.scrollTop = targetScrollTop > 0
          ? targetScrollTop
  ...
  }, [activeChat, currentChannel, currentConversationUiState.messageScrollTop, messages.length, mobileView]);
  ...
  }, [currentChannel?.id, isThreadCollapsed, persistCurrentConversationUiState, threadBlockId, threadMsgId, threadOrigin]);
  ...
  useEffect(() => {
    ...
    if (threadMessages.some((message) => message.id === editingThreadMessageId)) return;
    setEditingThreadMessageId(null);
    threadEditRestoreRef.current = null;
  ...
  useEffect(() => {
    ...
    if (threadConversation.messages.some((message) => message.id === threadMsgId)) return;
    closeThreadPane();
  }, [closeThreadPane, threadConversation, threadMsgId]);

  const activeChannels = useMemo(
    () => channels.filter((channel) => (channel.lifecycleStatus || 'flowing') === 'flowing'),
    [channels],
  );
  ...
  const listRows = useMemo<Array<{ type: 'channel'; channel: ChatChannel } | { type: 'folded-entry' }>>(() => {
    if (listViewMode === 'folded') {
      return [{ type: 'folded-entry' as const }, ...foldedChannels.map((channel) => ({ type: 'channel' as const, channel }))];
    }

    const rows: Array<{ type: 'channel'; channel: ChatChannel } | { type: 'folded-entry' }> = mainChannels.map((channel) => ({ type: 'channel' as const, channel }));
    if (foldedChannels.length === 0) return rows;
    rows.splice(Math.min(20, rows.length), 0, { type: 'folded-entry' as const });
    return rows;
  }, [foldedChannels, listViewMode, mainChannels]);

  const sortingStreams = useMemo(() => activeChannels.map((channel) => ({
    id: channel.id,
    title: channel.title,
    messages: channel.messages,
  })), [activeChannels]);
  const contextMenuMessage = useMemo(
    () => (contextMenu.msgId ? messages.find((message) => message.id === contextMenu.msgId) || null : null),
    [contextMenu.msgId, messages],
  );

  const { ref: contextMenuRef, pos: clampedContextMenuPos } = useClampedMenuPosition(
    contextMenu.x,
    contextMenu.y,
    [
      contextMenu.show,
  ...
  const resizeListPane = useCallback((delta: number) => {
    if (isListCollapsed) {
      if (delta > 0) {
        setIsListCollapsed(false);
        listPane.setSize(Math.min(LIST_PANE_MAX, Math.max(LIST_PANE_MIN, LIST_PANE_COLLAPSED_WIDTH + delta)));
      }
      return;
    }
  ...
  const resizeThreadPane = useCallback((delta: number) => {
    if (isThreadCollapsed) {
      if (delta < 0) {
        setIsThreadCollapsed(false);
        threadPane.setSize(Math.min(THREAD_PANE_MAX, Math.max(THREAD_PANE_MIN, THREAD_PANE_COLLAPSED_WIDTH - delta)));
      }
      return;
    }
  ...
  }) as CSSProperties, [isListCollapsed, isThreadCollapsed, listPane.size, threadPane.size]);
  ...
  const buildDraftPayload = useCallback((draft: DraftState) => ({
    ...draft,
  ...
  const prepareDraftPayload = useCallback(async (draft: DraftState) => {
  ...
  const appendMainBlocks = useCallback((blocks: BubbleBlock[]) => {
    if (blocks.length === 0) return;
    setCurrentDraft((prev) => updateDraftBlocks(prev, [...getDraftBlocks(prev), ...blocks]));
  }, [setCurrentDraft]);

  const appendThreadBlocks = useCallback((blocks: BubbleBlock[]) => {
    if (blocks.length === 0) return;
    updateCurrentThreadDraft((prev) => updateDraftBlocks(prev, [...getDraftBlocks(prev), ...blocks]));
  }, [updateCurrentThreadDraft]);

  const sendRawMessage = useCallback(async (message: MessageData) => {
    if (!currentChannel) return null;
  ...
  const handleSendAction = useCallback(async () => {
    try {
      ...
      showToast(`${editingMessageId ? '更新' : '发送'}失败：${getErrorMessage(error)}`);
    }
  }, [applyConversation, buildDraftPayload, currentChannel, currentDraft, editingMessageId, maybeTriggerAutoMachines, prepareDraftPayload, setCurrentDraft, showToast]);
  ...
  const handleSaveFactoryBot = useCallback(async (payload: Record<string, unknown>) => {
    try {
  ...
  const handleSaveFactoryProvider = useCallback(async (payload: Record<string, unknown>) => {
    try {
  ...
  const handleRunContextMachine = useCallback(async (botId: string) => {
    try {
  ...
  }, [contextMenu.blockId, contextMenu.msgId, currentChannel?.id, openThreadPane, showToast]);
  ...
  const handleThreadFiles = useCallback(async (files: File[]) => {
  ...
  const sendThreadReply = useCallback(async () => {
    try {
      ...
      showToast(`评论${editingThreadMessageId ? '更新' : '发送'}失败：${getErrorMessage(error)}`);
    }
  }, [
    applyConversation,
    buildDraftPayload,
  ...
  const handleTouchStart = useCallback((event: React.TouchEvent, msg: MessageData, blockId?: string, subIndex?: number) => {
  ...
  const handleMouseRightClick = useCallback((event: React.MouseEvent, msg: MessageData, blockId?: string, subIndex?: number) => {
  ...
  const cancelMainEdit = useCallback(() => {
    setEditingMessageId(null);
    setCurrentDraft(() => mainEditRestoreRef.current || createEmptyDraftState());
    mainEditRestoreRef.current = null;
  }, [setCurrentDraft]);

  const cancelThreadEdit = useCallback(() => {
    setEditingThreadMessageId(null);
    updateCurrentThreadDraft(() => threadEditRestoreRef.current || createEmptyDraftState());
    threadEditRestoreRef.current = null;
  }, [updateCurrentThreadDraft]);
  ...
  }, [currentChannel, currentDraft, currentThreadDraft, messages, openThreadPane, setCurrentDraft, showToast, threadMsgId, updateCurrentThreadDraft]);
  ...
  const onCopyBubbleLink = useCallback(async () => {
    ...
    await copyTextToClipboard(buildBubbleLink({
      conversationId: currentChannel.id,
      messageId: contextMenu.msgId,
      blockId: contextMenu.blockId,
    }), '已复制泡泡链接');
    setContextMenu((prev) => ({ ...prev, show: false }));
  }, [contextMenu.blockId, contextMenu.msgId, copyTextToClipboard, currentChannel?.id, showToast]);

  const scrollToMessage = useCallback((targetId: string, blockId?: string, options?: { pushNav?: boolean }) => {
  ...
  const jumpToMsg = useCallback((targetId: string, blockId?: string) => {
    if (scrollToMessage(targetId, blockId, { pushNav: true })) {
      return;
    }

    const targetChannel = channels.find((channel) => channel.messages.some((message) => message.id === targetId)) || null;
    if (!targetChannel) {
      showToast('消息未找到');
      return;
    }
  ...
  useEffect(() => {
    ...
    if (currentChannel?.id !== pendingJumpTarget.conversationId) return undefined;

    const timer = window.setTimeout(() => {
      const jumped = scrollToMessage(pendingJumpTarget.messageId, pendingJumpTarget.blockId, { pushNav: false });
      if (!jumped) {
        showToast('消息未找到');
      }
  ...
  const handleThreadJumpToMsg = useCallback((targetId: string, blockId?: string) => {
  ...
  const handleStreamPhotoSelect = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      if (event.target.files && event.target.files.length > 0) {
        const nextBlocks = await Promise.all(Array.from(event.target.files).map(classifyFile));
  ...
  const handleStreamFileSelect = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      if (event.target.files && event.target.files.length > 0) {
        const nextBlocks = await Promise.all(Array.from(event.target.files).map(classifyFile));
        appendMainBlocks(nextBlocks);
  ...
  const threadMsg = useMemo(
    () => (threadMsgId ? threadMessages.find((item) => item.id === threadMsgId) || null : null),
    [threadMessages, threadMsgId],
  );
  const threadReplies = useMemo(
    () => (threadMsgId
      ? threadSortedMessages.filter((item) => (
  ...
  const handleChatPaneDragOver = useCallback((event: React.DragEvent<HTMLDivElement>) => {
    if (activeChat !== 'assistant') return;
    event.preventDefault();
    setIsDragOver(true);
  }, [activeChat]);
  ...
  const handleChatPaneDrop = useCallback((event: React.DragEvent<HTMLDivElement>) => {
    if (activeChat !== 'assistant') return;
    event.preventDefault();
    setIsDragOver(false);
  ...
  const handleMainComposerPaste = useCallback((event: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    if (text && text.startsWith('http') && !hasDraftContent(currentDraft) && !text.match(/\s/)) {
      event.preventDefault();
      setPendingLink(text);
  ...
  const handleMainComposerDrop = useCallback((event: React.DragEvent<HTMLTextAreaElement>) => {
    const files = event.dataTransfer.files;
    if (!files.length) return;
    event.preventDefault();
  ...
  const handleMainComposerDragOver = useCallback((event: React.DragEvent<HTMLTextAreaElement>) => {
    event.preventDefault();
  }, []);

  const noop = useCallback(() => undefined, []);
  const handleCreateConversation = useCallback(() => { void addNewChat(); }, [addNewChat]);
  ...
  const listPaneProps = useMemo(() => (activeChat === 'assistant' ? {
    rows: listRows,
    listViewMode,
  ...
  const openFullscreenMedia = useCallback((src: string, type: 'img' | 'video') => {
    setFsSrc(src);
    setFsType(type);
    setFsOpen(true);
  }, []);

  const handleThreadOpenThread = useCallback((messageId: string, blockId?: string) => {
  ...
  const handleToggleLike = useCallback(async (messageId: string, blockId?: string) => {
    try {
      if (!currentChannel) return;
  ...
  const handleThreadToggleLike = useCallback(async (messageId: string, blockId?: string) => {
    try {
  ...
  const handleSelectForwardConversation = useCallback(async (conversationId: string) => {
    try {
      ...
      const targetChannel = channels.find((channel) => channel.id === conversationId);
      applyConversation(updated, { select: conversationId === currentChannel?.id });
      setForwardingMessageId(null);
      showToast(`已转发到「${targetChannel?.title || '目标泡泡流'}」`);
    } catch (error) {
      showToast(`转发失败：${getErrorMessage(error)}`);
    }
  }, [applyConversation, channels, currentChannel?.id, forwardingMessageId, showToast]);
  ...
  const sendSortingSourceMessage = async ({
    ...
    draft: {
      text: string;
      items: Array<{ type: 'text' | 'img' | 'video' | 'audio' | 'link' | 'file'; val: string; fileName?: string }>;
    };
  }) => {
  ...
  const updateCurrentStreamBasics = async ({ title, avatarPreset, avatarUrl }: { title: string; avatarPreset: string; avatarUrl: string }) => {
    try {
  ...
  const clearCurrentStreamMessages = async () => {
    try {
      if (!streamSettingsChannel) return;
      const confirmed = window.confirm(`清空「${streamSettingsChannel.title}」里的全部聊天记录？此操作不可撤销。`);
      if (!confirmed) return;
      const cleared = await clearChatMessages(streamSettingsChannel.id);
      applyConversation(cleared, { select: selectedChatId === streamSettingsChannel.id });
  ...
  const handleDeleteContextMessage = async () => {
    try {
      ...
      applyConversation(updated, { select: true });
      setHighlightedMsg(null);
      if (threadConversationId === currentChannel.id && threadMsgId && !updated.messages.some((item) => item.id === threadMsgId)) {
        closeThreadPane();
      }
      showToast('消息已删除');
    } catch (error) {
  ...
  const handleSaveUserAvatar = useCallback(async (payload: { avatarUrl: string }) => {
    try {
  ...
  const editingThreadMessage = editingThreadMessageId
    ? threadMessages.find((message) => message.id === editingThreadMessageId) || null
    : null;
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[linear-gradient(160deg,#f8faf6_0%,#eff4ee_100%)] px-6 py-10">
        <div className="max-w-xl rounded-[32px] border border-black/10 bg-white p-8 shadow-xl">
          <div className="text-[11px] uppercase tracking-[0.28em] text-[var(--text-secondary)]">Runtime Missing</div>
          <h1 className="mt-3 text-3xl font-bold text-[var(--text-primary)]">请从 Electron 启动桌面端</h1>
          <p className="mt-4 text-sm leading-7 text-[var(--text-secondary)]">开发时请运行 `npm run dev --workspace desktop`。</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <PrimaryTabBar
        ...
        onSelectChat={() => { setActiveTab('chat'); setActiveChat('assistant'); setMobileView('chat'); }}
        onSelectSorting={() => { persistCurrentConversationUiState({ immediate: true }); setActiveTab('sorting'); setActiveChat('sorting'); setMobileView('sorting'); }}
        onSelectFactory={() => { persistCurrentConversationUiState({ immediate: true }); setActiveTab('factory'); setActiveChat('factory'); setMobileView('factory'); }}
        onToggleList={() => setIsListCollapsed((prev) => !prev)}
      />

      <div className={`toast ${toastMsg ? 'show' : ''}`}>{toastMsg}</div>
      {fsOpen && (
        <div className="fullscreen-overlay show" onClick={() => { setFsOpen(false); fsVideoRef.current?.pause(); }}>
          <div className="fullscreen-close">×</div>
          {fsType === 'img'
            ? <img src={fsSrc} className="fullscreen-content" alt="fullscreen" />
            : <video src={fsSrc} className="fullscreen-content" controls ref={fsVideoRef} autoPlay onClick={(event) => event.stopPropagation()} />}
        </div>
      )}
      {contextMenu.show && (
        <div
        ...
        >
          <div className="context-menu-item" onClick={() => { void onCopyBubble(); }}>复制泡泡</div>
          <div className="context-menu-item" onClick={() => { void onCopyBubbleLink(); }}>复制泡泡链接</div>
          {contextMenuMessage?.role === 'me' ? (
            <div className="context-menu-item" onClick={() => { if (contextMenu.msgId) startEditingMessage(contextMenu.msgId); }}>编辑泡泡</div>
          ) : null}
          <div className="context-menu-item" onClick={() => { void onTriggerQuote(); }}>引用</div>
          <div className="context-menu-item" onClick={onTriggerComment}>评论</div>
          {manualMachineBots.length > 0 && contextMenu.msgId ? (
            <>
              <div className="context-menu-item" onClick={() => setContextMenuSubmenu((prev) => (prev === 'machines' ? null : 'machines'))}>
                <span>投入泡泡机</span>
                <span className={`context-menu-caret ${contextMenuSubmenu === 'machines' ? 'is-open' : ''}`}>›</span>
              </div>
                ...
                <div className="context-menu-subgroup">
                  {manualMachineBots.map((bot) => (
                    <button
                      ...
                      onClick={() => { void handleRunContextMachine(bot.id); }}
                    >
                      <span>{bot.name}</span>
                      <span>{bot.runtimeType === 'external-codex' ? 'Codex' : (bot.providerName || 'LLM')}</span>
                    </button>
                  ))}
                </div>
              ) : null}
          ...
          <div className="context-menu-item" onClick={() => { if (contextMenu.msgId) handleForwardMessage(contextMenu.msgId); setContextMenu((prev) => ({ ...prev, show: false })); }}>转发到...</div>
          <div className="context-menu-item" onClick={() => { void handleDeleteContextMessage(); }}>删除消息</div>
        </div>
      )}

      {forwardingMessageId ? (
        <div className="confirm-dialog-overlay" onClick={() => setForwardingMessageId(null)}>
            ...
            <div className="forward-picker-list">
              {forwardTargets.map((channel) => (
                <button
                  ...
                  onClick={() => { void handleSelectForwardConversation(channel.id); }}
                >
                  <div className="forward-picker-item__body">
                    <strong>{channel.title}</strong>
                    <span>{channel.lastMsg || '暂无内容'}</span>
                  </div>
                  {channel.id === currentChannel?.id ? <span className="forward-picker-item__badge">当前</span> : null}
                </button>
              ))}
            </div>
            <div className="confirm-actions">
              <button className="btn-secondary" onClick={() => setForwardingMessageId(null)}>取消</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {showFileConfirm && (
        <div className="confirm-dialog-overlay" onClick={() => setShowFileConfirm(false)}>
          <div className="confirm-dialog" onClick={(event) => event.stopPropagation()}>
            <div className="confirm-title">检测到文件/图片</div>
            <div className="confirm-desc">您希望直接发送这些内容，还是先放进当前泡泡的内容块托盘？</div>
            <div className="confirm-actions">
              <button className="btn-secondary" onClick={async () => {
                try {
                  setShowFileConfirm(false);
                  const blocks = await Promise.all(pendingFiles.map(classifyFile));
                  appendMainBlocks(blocks);
                  setPendingFiles([]);
                } catch (error) {
                  showToast(`处理文件失败：${getErrorMessage(error)}`);
              ...
              <button className="btn-primary" onClick={async () => {
                try {
                  ...
                  for (const file of pendingFiles) {
                    ...
                    const item = classifyMessageFile(file, url);
                    if (item.type === 'file') {
                      await sendRawMessage({ id: crypto.randomUUID(), role: 'me', type: 'file', content: { name: file.name, size: `${(file.size / 1024).toFixed(1)} KB`, url }, time: Date.now() });
                    } else {
                      await sendRawMessage({ id: crypto.randomUUID(), role: 'me', type: item.type as MessageData['type'], content: item.val, time: Date.now() });
                    }
                  }
                  setPendingFiles([]);
                } catch (error) {
      ...
      {showLinkConfirm && (
        <div className="confirm-dialog-overlay" onClick={() => setShowLinkConfirm(false)}>
          <div className="confirm-dialog" onClick={(event) => event.stopPropagation()}>
            <div className="confirm-title">检测到链接</div>
            <div className="confirm-desc">是否将此链接加入当前泡泡的内容块，或直接发送成单独泡泡？</div>
            <div className="confirm-actions">
              <button className="btn-secondary" onClick={() => {
                setShowLinkConfirm(false);
                if (pendingLink) {
                  setCurrentDraft((prev) => updateDraftBlocks(prev, [
                    ...getDraftBlocks(prev),
                    { id: crypto.randomUUID(), type: 'text', text: pendingLink },
                  ]));
                }
                setPendingLink(null);
              }}>作为文本</button>
              <button className="btn-primary" onClick={() => {
                ...
                if (pendingLink) {
                  setCurrentDraft((prev) => updateDraftBlocks(prev, [
                    ...getDraftBlocks(prev),
                    { id: crypto.randomUUID(), type: 'link', url: pendingLink },
                  ]));
                }
                setPendingLink(null);
              }}>加入内容块</button>
